...
\label{chap:basarim}
\bolumfoto{gorseller/bolum02-dolmabahce.jpg}{Dolmabahçe Sarayı Saat Kulesi -- İstanbul}
...
Mobil                 & Pil ömrü, tepki süresi   & Termal kısıt         & FLOPS/watt \\
...
\]
%
yol alabilir --- bir ana kartın uzun kenarından bile kısadır! Üstelik işlemcinin içindeki sinyaller boşlukta değil, bakır teller üzerinde ilerler; bakırda yayılma hızı ışık hızının yaklaşık üçte ikisine (${\sim}\,2 \times 10^{8}$\,m/s) düşer ve bir çevrimdeki menzil yalnızca ${\sim}\,5$\,cm'ye iner. Mantık kapılarının kendi anahtarlama gecikmeleri de bu bütçeden pay aldığında, kritik yol üzerindeki fiziksel tel uzunluğu birkaç santimetreyi geçemez.

Bu fiziksel gerçek doğrudan bir tasarım ilkesine yol açar: bir birimi küçültmek, içindeki telleri kısaltır ve sinyal gecikmesini azaltır; dolayısıyla daha yüksek saat sıklığına olanak tanır. Tersine, büyük bir yazmaç dosyası veya geniş bir çoklayıcı daha uzun teller ve daha fazla kapı gecikmesi demektir --- fiziksel boyut doğrudan hızı sınırlar.

\begin{tanim}[Tasarım İlkesi 1: Küçük olan hızlıdır]
\index{tasarım ilkeleri!küçük olan hızlıdır}
Donanımda küçük yapılar büyük yapılardan daha hızlı çalışır. Daha az sayıda eleman, daha kısa bağlantı hatları ve daha az kapı gecikmesi anlamına gelir. Bu ilke, yazmaç dosyasının boyutundan önbellek büyüklüğüne, boru hattı aşamalarının karmaşıklığından veri yolu genişliğine kadar pek çok mimari kararı doğrudan etkiler. Tabii ki ``küçük'' her zaman ``daha iyi'' demek değildir: az sayıda yazmaç derleyicilerin elini bağlar, küçük bir önbellek isabet oranını düşürür. İyi bir tasarım, küçüklüğün hız avantajı ile büyüklüğün işlevsel avantajı arasında doğru dengeyi bulmaktır.
\end{tanim}
...
\begin{table}[H]
\centering
\caption[Aynı C kodunun üç farklı BKM'de derlenmesi]{Tek satırlık bir C ifadesinin (\texttt{puan = hasar[tur] * carpan}) üç farklı BKM'de derlenmiş hâli. Buyruk ayrıntılarını Bölüm~\ref{chap:buyruk-kumesi}'de öğreneceğiz; burada önemli olan buyruk sayısı ve türlerindeki farktır.}
...
\begin{terimnotu}[BBÇ — Buyruk Başına Çevrim]
İngilizcede \emph{CPI — Cycles Per Instruction} olarak bilinen bu ölçüt, bir buyruğun yürütülmesi için harcanan ortalama saat çevrimi sayısını verir. Kitapta kısaca \textbf{BBÇ} olarak kullanılmaktadır.
\end{terimnotu}

\textbf{Buyruk Başına Çevrim}\index{buyruk başına çevrim}\index{BBÇ}\index{CPI|see{BBÇ}} (BBÇ), programın çalıştırılması için gereken buyruklardan her birisinin yürütülmesi için geçen ortalama çevrim sayısıdır:
...
\begin{itemize}
    \item \textbf{EEMBC}\index{EEMBC} (Embedded Microprocessor Benchmark Consortium): Gömülü sistemler ve mikrodenetleyiciler için tasarlanmış sınama kümeleri sunar. \emph{CoreMark}\index{CoreMark}~\cite{galon2012} programı bu konsorsiyumun en bilinen ürünüdür ve gömülü işlemci karşılaştırmalarında fiilen standart hâline gelmiştir.
...
\begin{tikzpicture}[>=Stealth]
    ...
    \node[font=\scriptsize\bfseries, anchor=south west, text=sinyalyesil] at (1.3, 1.9) {Mobil (ARM)};
...
\begin{itemize}
    ...
    \item \textbf{Enerji verimliliği:} Apple M3 Max, 92\,W ile diğerlerinin yarısı kadar güç harcayarak yakın başarım sunar. Bu, ARM mimarisinin enerji verimliliği avantajını somut olarak gösterir.
...
\subsubsection*{Tuzak: Tepe başarımı gerçek başarım sanmak}
...
\subsubsection*{Tuzak: Amdahl Yasası'nı göz ardı etmek}
...
\subsubsection*{Yanılgı: Transistör sayısı arttıkça başarım aynı oranda artar}
...
\subsubsection*{Yanılgı: Enerji verimli işlemci her zaman yavaştır}
...
Bu bölümde bilgisayar başarımını tanımlayan, ölçen ve sınırlayan temel kavramları inceledik. Aşağıdaki tablo bölümün ana hatlarını özetlemektedir:
...
\midrule
Yürütme zamanı & $= \text{Buyruk sayısı} \times \text{BBÇ} \times \text{Çevrim zamanı}$. Üç bileşen birbirine bağlıdır; birini iyileştirmek diğerini kötüleştirebilir. \\
İşlem hacmi vs.\ gecikme & Gecikme tek görevin süresini, işlem hacmi birim zamandaki görev sayısını ölçer. Sunucu iş yükleri işlem hacmini, etkileşimli uygulamalar gecikmeyi önceliklendirir. \\
MIPS / MFLOPS & Yanıltıcı olabilir; farklı BKM'ler ve buyruk karışımları karşılaştırmayı anlamsız kılar. \\
FLOPS & Bilimsel ve YZ hesaplamalarında temel ölçüt. Duyarlık düzeyine dikkat edilmeli; tepe ve sürdürülebilir başarım ayrılmalı. \\
SPEC / MLPerf & Çeşitli iş yükleri üzerinde tekrarlanabilir, adil karşılaştırma imkânı sunar. \\
Amdahl Yasası & $\text{Hızlanma} = \frac{1}{(1-P)+P/H}$. Bir iyileştirme programın tamamını değil yalnızca $P$ oranındaki kısmını hızlandırır; geri kalan kısım toplam hızlanmaya üst sınır koyar. ``Olağan durumu hızlandır'' ilkesinin nicel temelidir. \\
Gustafson Yasası & $S(N) = N - s(N-1)$. Çekirdek sayısı arttıkça problem büyüklüğü de artırılırsa Amdahl'ın karamsar sınırı aşılabilir. \\
Bellek duvarı & İşlemci ile bellek arasındaki büyüme hızı farkı; önbellek hiyerarşisi ve HBM ile hafifletilir. \\
Güç-başarım & Gerilimin karesiyle orantılı güç tüketimi (Bölüm~\ref{sec:devingen-guc}), saat sıklığı yerine koşutluğa yönelmenin temel nedeni. Green500 sıralaması başarım/watt ölçütünü kullanır. \\
\bottomrule
\end{tabular}
\end{center}

Başarım eğilimleri, 2005'e kadar süren yılda \%50'lik tek çekirdek büyümesinin güç duvarıyla birlikte \%3--5'e gerilemesiyle köklü bir dönüşüm geçirmiştir. Günümüzde başarım artışı çok çekirdekli işlemciler, alana özgü hızlandırıcılar ve gelişmiş bellek hiyerarşileriyle sağlanmaktadır. Yaygın yanılgılar --- saat sıklığının tek başına belirleyici olduğu, MIPS'in güvenilir olduğu, tepe başarımın sürdürülebilir olduğu --- hem sınavlarda hem gerçek tasarım kararlarında sıkça karşılaşılan tuzaklardır.
...
\begin{enumerate}[(a)]
    ...
    \item L1 önbellek erişim süresi 1\,ns ise 2025 için önbellek isabet oranının en az ne kadar olması gerekir ki ortalama bellek erişim süresi 10 çevrimden az olsun?
\end{enumerate}
\end{alistirma}
