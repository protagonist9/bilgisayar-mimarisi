...
Giriş aygıtlarının hızları birbirinden çok farklıdır. Bir klavye saniyede birkaç bayt üretirken, bir mikrofon saniyede on binlerce ses örneği (\emph{sample}) üretir --- örneğin 16 bit, 48\,kHz örnekleme ile saniyede yaklaşık 96\,KB. Yüksek çözünürlüklü bir kamera ise saniyede gigabaytlar düzeyinde ham veri üretebilir. Bu büyük hız farkı, giriş/çıkış alt sisteminin tasarımında can alıcı bir etkendir ve Bölüm~\ref{chap:giris-cikis}'de ayrıntılı olarak ele alınacaktır.
...
\end{equation}
%
FIT birimi özellikle yarıiletken endüstrisinde yaygın olarak kullanılır. Bir bellek yongasının FIT değeri 100 ise, bu yonga bir milyar saat çalışmada ortalama 100~kez arıza verir --- ya da eşdeğer olarak, 10 milyon saat çalışmada 1~kez arıza beklenir.
...
Bu üç kavram birbirini tamamlar: güvenilir bir sistem uzun süre doğru çalışır; dayanıklı bir sistem fiziksel koşullara direnir; gürbüz bir sistem beklenmedik durumlarla başa çıkar. İyi bir tasarım üçünü birden gözetir.
...
RISC-V'in açık kaynak, modüler ve eğitim dostu yapısı, onu hem bu kitabın hem de dünya genelinde giderek artan sayıda mimari projenin tercih ettiği buyruk kümesi haline getirdi. Yapay zekâ ise bilgisayar mimarisinin yeni ufku: GPU, TPU ve NPU gibi alana özgü hızlandırıcılar, genel amaçlı MİB'lerin tek başına karşılayamadığı devasa hesaplama gereksinimlerini verimli biçimde karşılamak için tasarlanıyor. Bu hızlandırıcıların mimarisi Bölüm~\ref{chap:gpu-hizlandiricilar}'da kapsamlı olarak incelenecek.
...
\begin{alistirma}[\orta]
Bir mobil işlemcinin durağan (sızıntı) güç tüketimi $P_{\text{durağan}} = I_{\text{sızıntı}} \times V_{dd}$ formülüyle hesaplanır. İşlemci 0{,}75\,V kaynak geriliminde çalışmaktadır ve toplam sızıntı akımı 2{,}5\,A'dir.
\begin{enumerate}[(a)]
