...
buyruk düzeyi koşutluğu & instruction-level parallelism (\texttt{ILP}) \\
buyruk biçimi & instruction format \\
buyruk getirme birimi & instruction fetch unit \\
buyruk kümesi mimarisi (\texttt{BKM}) & Instruction Set Architecture (\texttt{ISA}) \\
...
Tek Yongada Sistem (\texttt{SoC}) & System on Chip (\texttt{SoC}) \\
...
instruction-level parallelism (\texttt{ILP}) & buyruk düzeyi koşutluğu \\
...
System on Chip (\texttt{SoC}) & Tek Yongada Sistem (\texttt{SoC}) \\
